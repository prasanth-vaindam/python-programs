...
# version 5: adding attempt logs of all
    ...
    for i in range(1, num_questions + 1):
        ...
        if user_answer == correct_answer:
            if time_taken > TIME_THRESHOLD:
                print(f"✅ Correct, but slow! (Time: {time_taken}s)")
                log_slow(player_name, a, b, time_taken)
            else:
...
def log_mistake(player_name, a, b, user_answer, correct_answer):
    with open(MISTAKE_LOG, "a") as f:
        f.write(f"{player_name} missed: {a}×{b} = {correct_answer}, answered: {user_answer} (Table {a})\n")


def log_slow(player_name, a, b, time_taken):
    with open(SLOW_LOG, "a") as f:
        f.write(f"{player_name} took {time_taken}s for {a}×{b} (Table {a}) – slow response\n")
...
def show_leaderboard():
    ...
    for idx, (name, stars, t) in enumerate(players[:20], start=1):  # top 10
